...
parser.add_argument('--vis_mode', action='store_true', default=False, help='use visualize mode')
...
parser.add_argument("--limit_param", default=999900000, type=int, help="upper limit of params")
...
def main():
  ...
  # args.save = './test_result/mult2/search-{}_{}_target3/target={}_lambda={}_gamma={}_epochs={}/'.format(args.save_dir,args.set, args.limit_param, args.lambda_a,args.gammas_learning_rate,args.epochs)
  args.save = './test_result2/pscore_vil/lambda/target_{}_id_{}_lambda={}/'.format(args.limit_param,args.id,args.lambda_a)
  ...
  with open(csv_file_path_param_ite, 'w') as f:
    ...
    writer.writerow(['param','train_loss','train_acc','pscore','val_loss','epoch','model'])
  ...
  for epoch in range(1, args.epochs + 1):
    ...
    if args.vis_mode:
      vis.plot(genotype.normal, "normal", epoch,pytorch_total_params_train,args.lambda_a,args.limit_param,args.id)
      vis.plot(genotype.reduce, "reduce", epoch,pytorch_total_params_train,args.lambda_a,args.limit_param,args.id)
  ...
  if args.train_mode:
    ...
    training.main(best_genotype,args.layers)
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    if epoch>=15:
      ...
      if pytorch_total_params_train > limit_param :
        ...
        # if step == 0:
        num_flag = 1
        train_param, genotype,pscore,val_loss = architect.param_step(input, target, input_search, target_search, lr, optimizer,pytorch_total_params_train,step,limit_param, num_flag,lambda_a,param_prev)
        ...
        model = model_copy
        val_loss = architect.step(input, target, input_search, target_search, lr, optimizer,pytorch_total_params_train,step,limit_param,num_flag, unrolled=args.unrolled)
        # top1_val.update(acc_x.data.item(), n)
    ...
    with open(csv_file_path_param_ite, 'a') as f:
      writer = csv.writer(f)
      writer.writerow([param_prev,objs.avg,top1.avg,pscore,val_loss,epoch,genotype])
