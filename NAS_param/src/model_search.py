...
class Network(nn.Module):
  ...
  def _initialize_alphas(self,device):
    ...
    # self.cell_gate1 = Variable(1e-3*torch.randn(num_layers).to(device), requires_grad=True)
    # self.cell_gate2 = Variable(1e-3*torch.randn(num_layers).to(device), requires_grad=True)
